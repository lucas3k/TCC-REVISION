...
export default function QuizViagem() {
  ...
  useEffect(() => {
    readItem('fullViagem').then((data) => {
      if (data) {
        const fullViagem = JSON.parse(data);

        setPassagens(fullViagem.valores[0]);
        setAcomodacao(fullViagem.valores[1]);
        setAlimentacao(fullViagem.valores[2]);
        setPasseio(fullViagem.valores[3]);
        settransporte(fullViagem.valores[4]);
        setdocumentacao(fullViagem.valores[5]);
        setSeguro(fullViagem.valores[6]);
        setEmergencia(fullViagem.valores[7]);
        setCompras(fullViagem.valores[8]);
        setOutro(fullViagem.valores[9]);
        ...
        <View style={styles.container}>
          ...
          <TouchableOpacity style={styles.botaoSalvar}
            onPress={() => {
              ...
              createItem('fullViagem', JSON.stringify(fullViagem));
