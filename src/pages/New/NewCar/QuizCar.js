...
export default function QuizCar() {
  ...
  useEffect(() => {
    readItem('fullCarro').then((data) => {
      if (data) {
        const fullCarro = JSON.parse(data);

        setValor(fullCarro.valores[0]);
        setSeguro(fullCarro.valores[1]);
        setLicenca(fullCarro.valores[2]);
        setDocumentos(fullCarro.valores[3]);
        setManutencao(fullCarro.valores[4]);
        setTaxa(fullCarro.valores[5]);
        setFinanciamento(fullCarro.valores[6]);
        ...
        <View style={styles.container}>
          ...
          <TouchableOpacity style={styles.botaoSalvar}
            onPress={() => {
              ...
              createItem('fullCarro', JSON.stringify(fullCarro));
