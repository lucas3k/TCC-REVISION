...
export default function QuizCasa() {
  ...
  useEffect(() => {
    readItem('fullCasa').then((data) => {
      if (data) {
        const fullCasa = JSON.parse(data);

        setAreia(fullCasa.valores[0]);
        setPedra(fullCasa.valores[1]);
        setCimento(fullCasa.valores[2]);
        setFerro(fullCasa.valores[3]);
        setArgamassa(fullCasa.valores[4]);
        setTijolo(fullCasa.valores[5]);
        setMadeira(fullCasa.valores[6]);
        setTelha(fullCasa.valores[7]);
        setVidro(fullCasa.valores[8]);
        setLuz(fullCasa.valores[9]);
        setPiso(fullCasa.valores[10]);
        setAcabamento(fullCasa.valores[11]);
        setPintura(fullCasa.valores[12]);
        setMao(fullCasa.valores[13]);
        setOutro(fullCasa.valores[14]);
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        <View style={styles.container}>
          ...
          <TouchableOpacity style={styles.botaoSalvar}
            onPress={() => {
              const fullCasa = {
                vTotal: calcularTotal(),
                valores: [areia, pedra, cimento, ferro, argamassa, tijolo, madeira, telha, vidro, luz, piso, acabamento, pintura, mao, outro]
              }

              createItem('fullCasa', JSON.stringify(fullCasa));
